...
//				var style = styleValue ? 'style="' + styleValue + '"' : '';
	...
	onBeforeRender: function(target, rows){
		...
		for(var i=0; i<rows.length; i++){
			var row = rows[i];
			//修改后的代码  修改原因：当分组时的关键字为一个对象中的一个属性时需要以该对象的属性来分组这样传值就传属性加点然后加属性名 ，如department为一个组对象，要以该组对象department的name属性来分组，那么就传值department.name
			...
			if(groupField.indexOf('.')!=-1){
				pre = groupField.split('.')[0];
				suf = groupField.split('.')[1];
				group = getGroup(row[pre][suf]); 
				key = row[pre][suf];
			}
			if (!group){
				group = {
